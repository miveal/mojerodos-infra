...
**Status:** partial — two roots scaffolded + import-verified (0 drift); NOT yet applied/adopted
**Verified as of:** 2026-07-14 on commit `2481c88` (working tree; not yet committed)
...
- **First apply not yet run.** Nothing is in state. Needs an **edit-scoped** token + the S3
  backend (AWS creds). After each root's apply, delete its `imports.tf`.
- **`CLOUDFLARE_API_TOKEN` repo secret** must be created (edit scope) before CI can plan/apply —
  the PR `plan` job calls the CF API and will fail without it.
